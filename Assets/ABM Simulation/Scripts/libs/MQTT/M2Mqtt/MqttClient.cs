...
namespace uPLibrary.Networking.M2Mqtt
{
    ...
    public class MqttClient
    {
        ...
        // event for peer/MQTTSimClient disconnection
        ...
        /// <summary>
        /// Connection status between MQTTSimClient and broker
        /// </summary>
        ...
        {
            return this.Connect(clientId, null, null, false, MqttMsgConnect.QOS_LEVEL_AT_MOST_ONCE, false, null, null, true, MqttMsgConnect.MAX_KEEP_ALIVE);
        }
        ...
        public byte Connect(string clientId,
            ...
            string password)
        {
            return this.Connect(clientId, username, password, false, MqttMsgConnect.QOS_LEVEL_AT_MOST_ONCE, false, null, null, true, MqttMsgConnect.MAX_KEEP_ALIVE);
        }
            ...
            ushort keepAlivePeriod)
        {
            ...
            {
                // set all MQTTSimClient properties
        ...
        private MqttMsgPingResp Ping()
        {
            ...
            catch (Exception e)
            {
                ...
                // MQTTSimClient must close connection
                this.OnConnectionClosing();
                return null;
            }
        }
        ...
        {
            this.MqttMsgPublishReceived?.Invoke(this,
    new MqttMsgPublishEventArgs(publish.Topic, publish.Message, publish.DupFlag, publish.QosLevel, publish.Retain));
        ...
        {
            this.MqttMsgPublished?.Invoke(this,
    new MqttMsgPublishedEventArgs(messageId, isPublished));
        ...
        {
            this.MqttMsgSubscribed?.Invoke(this,
    new MqttMsgSubscribedEventArgs(suback.MessageId, suback.GrantedQoSLevels));
        ...
        {
            this.MqttMsgUnsubscribed?.Invoke(this,
    new MqttMsgUnsubscribedEventArgs(messageId));
        ...
        /// <summary>
        /// Wrapper method for peer/MQTTSimClient disconnection
        /// </summary>
        ...
        private bool EnqueueInflight(MqttMsgBase msg, MqttMsgFlow flow)
        {
                ...
                (msg.QosLevel == MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE))
            {
                lock (this.inflightQueue)
                {
                    ...
                    //        to/from MQTTSimClient and message id could be the same (one tracked by broker and the other by MQTTSimClient)
            ...
            if (enqueue)
            {
                ...
                // based on QoS level, the messages flow between broker and MQTTSimClient changes
                switch (msg.QosLevel)
                {
                    // QoS Level 0
...
#else
                if ((this.eventQueue.Count == 0) && !this.isConnectionClosing)
                    // wait on receiving message from MQTTSimClient
                    this.receiveEventWaitHandle.WaitOne();
#endif

                // check if it is running or we are closing MQTTSimClient
                if (this.isRunning)
                {
                    ...
                    if (internalEvent != null)
                    {
                        ...
                        if (msg != null)
                        {
                            switch (msg.Type)
                            {
                                ...
                                // UNSUBSCRIBE message received from MQTTSimClient
                                case MqttMsgBase.MQTT_MSG_UNSUBSCRIBE_TYPE:
                                ...
                                // DISCONNECT message received from MQTTSimClient
                                case MqttMsgDisconnect.MQTT_MSG_DISCONNECT_TYPE:
                    ...
                    if ((this.eventQueue.Count == 0) && this.isConnectionClosing)
                    {
                        ...
                        // MQTTSimClient raw disconnection
                        this.OnConnectionClosed();
                    }
                }
            }
        ...
        private void ProcessInflightThread()
        {
                    ...
                    if (this.isRunning)
                    {
                        lock (this.inflightQueue)
                        {
                            ...
                            while (count > 0)
                            {
                                ...
                                // check to be sure that MQTTSimClient isn't closing and all queues are now empty !
                                if (!this.isRunning)
                                    break;
            ...
            catch (MqttCommunicationException e)
            {
                ...
                // raise disconnection MQTTSimClient event
                this.OnConnectionClosing();
            }
        }
